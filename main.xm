...
This is an old project and there are a lot of changes in iOS / tvOS since it was maintained, it was also written with pretty much 
zero error / nil checking and it makes HEAVY use of foundation classes & methods that throw execeptions when they receive nil
to combat this- do a HEAVY amount of swizzling and return values that will allow execution to continue while attempting
to skip past the troublesome code.
...
#if TARGET_OS_TV
@interface FBWorkspaceConnectionListener: NSObject
+ (id) _sharedDomain;
@end

@interface FBWorkspaceConnectionListener (crashfixer)
+ (id) _ourSharedDomain;
@end
@implementation FBWorkspaceConnectionListener (crashfixer)
+ (id) _ourSharedDomain {
    return @"kek";
    //this doesnt work
    @try 
    {
        id orig = [self _ourSharedDomain];
        return orig; 
    }
    @catch (NSException * e) 
    {
...
@end
@interface SpringBoardUI (crashfixer)
+ (void)ourLoad; 
@end
@implementation SpringBoardUI (crashfixer)
+ (void)ourLoad {
    NSLog(@"FUCKKK YOUOOO");
} 
@end
...
@interface NSFileManager (crashfixer)
- (BOOL)ourCopyItemAtURL:(NSURL *)srcURL toURL:(NSURL *)dstURL error:(NSError **)error;
@end
@implementation NSFileManager (crashfixer)
- (BOOL)ourCopyItemAtURL:(NSURL *)srcURL toURL:(NSURL *)dstURL error:(NSError **)error {
    if (srcURL == nil || dstURL == nil ){
        NSLog(@"CAUGHT_CRASH nil url: %@ or %@", srcURL, dstURL);
        return self;
    }
    return [self ourCopyItemAtURL:srcURL toURL:dstURL error:error];
}
@end
@interface NSURL (crashfixer)
- (NSURL *)OURURLByAppendingPathComponent:(NSString *)pathComponent;
@end
@implementation NSURL (bro)
- (NSURL *)OURURLByAppendingPathComponent:(NSString *)pathComponent {
    if (pathComponent == nil){
        NSLog(@"CAUGHT_CRASH nil component: %@", self);
        return self;
    }
    return [self OURURLByAppendingPathComponent:pathComponent];
}
@end

@interface NSString (crashfixer)
- (NSString *)identifier;
- (NSString *)ourSubstringWithRange:(NSRange)range;
- (NSString *)ourSubstringToIndex:(NSInteger)index;
+ (instancetype)ourStringWithUTF8String:(const char *)cString;
- (NSString *)ourStringByAppendingString:(NSString *)string ;
@end

@implementation NSString (crashfixer)

//in one of my "fixes", im not returning the right kind of object, this moves things along."
- (NSString *)identifier {
    return nil;//@"com.apple.frontboard.workspace-service";
}

- (NSString *)ourSubstringWithRange:(NSRange)range {
    //NSLog(@"%@ with range: %@", self, NSStringFromRange(range));
    if ([self length] < range.length){
        //NSLog(@"CAUGHT_CRASH length of: %@ which is %lu less than range.length: %lu", self, self.length, range.length);
        return @"";
    }
    return [self ourSubstringWithRange:range];
}

- (NSString *)ourSubstringToIndex:(NSInteger)index {
    if ([self length] < index){
        //NSLog(@"CAUGHT_CRASH %@ length %lu is < ourSubstringToIndex: %lu", self, self.length, index);
        return @"";
    }
    return [self ourSubstringToIndex:index];
}

- (NSString *)ourStringByAppendingString:(NSString *)string {
    if (string == nil){
        //NSLog(@"CAUGHT_CRASH %@ ourStringByAppendingString: %@", self, string);
        return self;
    }
    return [self ourStringByAppendingString:string];
}

+ (instancetype)ourStringWithUTF8String:(const char *)cString {
    
    //NSLog(@"CAUGHT_CRASH stringWithCString");
    if (cString == NULL) {
        return NULL;
    }
    //fprintf(stderr, "\nstringWithUTF8String: %s\n", cString);
    return [self ourStringWithUTF8String:cString]; // call original
}
@end

/*

@interface NSArray (crashfixer)
- (void)ourRemoveObjectsInRange:(NSRange)range;
- (id)ourInitWithObjects:(id)obj count:(NSInteger)count;
@end

@implementation NSArray (crashfixer)
- (void)ourRemoveObjectsInRange:(NSRange)range{
    
    if (range.length >= self.count || range.location == NSNotFound ) return;
    [self ourRemoveObjectsInRange:range];
}

- (id)ourInitWithObjects:(id)objs count:(NSInteger)count {
    if (objs == nil) {
        NSLog(@"CAUGHT_CRASH init With Objects; %@ count: %lu", objs, count);
        
        return @[];
        return nil; 
    }
    return [self ourInitWithObjects:objs count:count];
}
@end
...
static void list_dir(const char *dir_name,BOOL writeToDisk,NSString *outputDir,BOOL getSymbols,BOOL recursive,BOOL simpleHeader,BOOL skipAlreadyFound,BOOL skipApplications){
    ...
    while (1) {
        ...
        printf("\n\033[F\033[J");
        int fwPos = locationOfString(dir_name,".framework/Frameworks");
        int supPos = locationOfString(dir_name,".framework/Support");
        //printf("fwPos: %i...\nsupPos: %i...\n",fwPos, supPos); 
        if (fwPos != -1 || supPos != -1){
            //break;
        }
        ...
        if (strcmp(d_name,".") && strcmp(d_name,"..")){
            ...
            NSString *currentPath=[NSString stringWithUTF8String:dir_name ];
            currentPath=[currentPath stringByReplacingOccurrencesOfString:@"//" withString:@"/"];
            NSString *currentFile=[NSString stringWithUTF8String:d_name ];
...
extern "C" int parseImage(char *image,BOOL writeToDisk,NSString *outputDir,BOOL getSymbols,BOOL isRecursive,BOOL buildOriginalDirs,BOOL simpleHeader, BOOL skipAlreadyFound,BOOL skipApplications){
    ...
    if (isRecursive && ([[NSString stringWithUTF8String:image ] rangeOfString:@"/dev"].location==0 || [[NSString stringWithUTF8String:image ] rangeOfString:@"/bin"].location==0 || (skipApplications && [[NSString stringWithUTF8String:image ] rangeOfString:@"/var"].location==0))){
        [xd drain];
        return 4;
    }
    [xd drain];
    ...
    if ([allImagesProcessed containsObject:[NSString stringWithUTF8String:image ]]){
        [xdd drain];
        return 5;
    }
    
    NSString *imageEnd=[[NSString stringWithUTF8String:image ] lastPathComponent];
    ...
    imageEnd=[imageEnd stringByReplacingOccurrencesOfString:@".app" withString:@""];
    NSString *containedImage=[[NSString stringWithUTF8String:image ] stringByAppendingString:[NSString stringWithFormat:@"/%@",imageEnd]];
    ...
    if (image!=nil && ![allImagesProcessed containsObject:[NSString stringWithUTF8String:image]] && ((dlopenError && (strstr(dlopenError,"no matching architecture in universal wrapper") || strstr(dlopenError,"out of address space") || strstr(dlopenError,"mach-o, but wrong architecture"))) || (isExec && !shouldDLopen32BitExecutables))){
        @autoreleasepool{
            
            /*if (fileExistsOnDisk(image) && isExec){
             NSString *exec=[NSString stringWithFormat:@"/usr/bin/ldid -e %s > /tmp/entitlements/%@",image,[[NSString stringWithUTF8String:image encoding:NSUTF8StringEncoding] lastPathComponent]];
            ...
            [allImagesProcessed addObject:[NSString stringWithUTF8String:image ]];
            RunCmd([tryWithLib UTF8String]);
        }
        if (!isRecursive){
            return 1;
        }
    ...
    if (image!=nil && [allImagesProcessed containsObject:[NSString stringWithUTF8String:image ]]){
        return 5;
    }
    ...
    NSString *imageName=[[NSString stringWithUTF8String:image ] lastPathComponent];
    NSString *fullImageNameInNS=[NSString stringWithUTF8String:image];
    ...
    [processedImages addObject:[NSString stringWithUTF8String:image ]];
    ...
    for (unsigned i=0; i<count; i++){
        ...
        NSArray *blackList = @[@"SKAction", @"BWAudioFormat", @"BWCrossoverNode", @"CMMTLDevice", @"_INPBBillPayeeValue", @"_INPBEventList", @"_INPBConfigureHomeIntent"];
        
        NSString *theName = [NSString stringWithUTF8String:names[i]];
        
        if ([blackList containsObject:theName]){
            CDLog(@"Skipping Class %s\n",names[i]);
            continue;
        }
        ...
        NSString *classNameNS=[NSString stringWithUTF8String:names[i]];
        ...
        for (unsigned t=0; t<protocolCount; t++){
            ...
            NSString *addedProtocol=[[NSString stringWithUTF8String:protocolName] retain];
        ...
        if (writeToDisk && superclassString.length>0 && ![superclassString isEqual:@" : NSObject"]){
            ...
            if (!simpleHeader){
                ...
                if (imageNameOfSuper){
                    NSString *imageOfSuper=[NSString stringWithUTF8String:imageNameOfSuper];
        ...
        for (unsigned d=0; d<protocolCount; d++){
            ...
            NSString *protocolNSString=[NSString stringWithUTF8String:protocolName];
            if (writeToDisk){
                if (simpleHeader){
                    ...
                    imageOfProtocol=([imagePrefix isEqual:protocolPrefix] || !class_getImageName(protocol) ) ? imageName : [NSString stringWithUTF8String:class_getImageName(protocol) ];
        ...
        if (ivarOutCount>0){	
            ...
            for (unsigned x=0;x<ivarOutCount;x++){
                ...
                NSString *ivarNameNS=[NSString stringWithUTF8String:ivarName];
                const char * ivarType=ivar_getTypeEncoding(currentIvar);
                
                NSString *ivarTypeString=commonTypes([NSString stringWithUTF8String:ivarType],&ivarNameNS,YES);
        ...
        for (unsigned int b=0; b<propertiesCount; b++){
            ...
            NSString *newString=propertyLineGenerator([NSString stringWithUTF8String:attrs],[NSString stringWithUTF8String:propname]);
        ...
        if (writeToDisk && [classesInClass count]>0){
            ...
            [classesInClass removeObject:[NSString stringWithUTF8String:names[i] ]];
        ...
        if (writeToDisk){
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:writeDir withIntermediateDirectories:YES attributes:nil error:nil];
            NSString *fileToWrite=[NSString stringWithUTF8String:names[i] ];
            
            if ([[NSString stringWithUTF8String:names[i] ] isEqual:[[NSString stringWithUTF8String:image ] lastPathComponent]]){
                fileToWrite=[[NSString stringWithUTF8String:names[i] ] stringByAppendingString:@"-Class"];
            }
    ...
    /*
     dlopen("/usr/lib/system/libdyld.dylib", RTLD_LAZY);
     MSImageRef libdyld = MSGetImageByName("/usr/lib/system/libdyld.dylib");
     void* dyld_all_your_infos = MSFindSymbol(libdyld, "__dyld_get_all_image_infos");
     NSLog(@"found %p", dyld_all_your_infos);
     */
...
int main(int argc, char **argv, char **envp) {
    ...
    @autoreleasepool {
        ...
        swizzleScience = [NSString jr_swizzleClassMethod:@selector(stringWithUTF8String:) withClassMethod:@selector(ourStringWithUTF8String:) error:&theError];
        
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSString stringWithUTF8String: replaced!");
        } else {
            
            NSLog(@"NSString stringWithUTF8String: failed to replace with error(: %@", theError);
        }
        swizzleScience = [NSString jr_swizzleMethod:@selector(stringByAppendingString:) withMethod:@selector(ourStringByAppendingString:) error:&theError];
        
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSString stringByAppendingString: replaced!");
        } else {
            
            NSLog(@"NSString stringByAppendingString: failed to replace with error(: %@", theError);
        }
        
        swizzleScience = [NSString jr_swizzleMethod:@selector(substringToIndex:) withMethod:@selector(ourSubstringToIndex:) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSString substringToIndex: replaced!");
        } else {
            
            NSLog(@"NSString substringToIndex: failed to replace with error(: %@", theError);
        }
        
        swizzleScience = [NSString jr_swizzleMethod:@selector(substringWithRange:) withMethod:@selector(ourSubstringWithRange:) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSString substringWithRange: replaced!");
        } else {
            
            NSLog(@"NSString substringWithRange: failed to replace with error(: %@", theError);
        }
        
        swizzleScience = [NSURL jr_swizzleMethod:@selector(URLByAppendingPathComponent:) withMethod:@selector(OURURLByAppendingPathComponent:) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSURL URLByAppendingPathComponent: replaced!");
        } else {
            
            NSLog(@"NSURL URLByAppendingPathComponent: failed to replace with error(: %@", theError);
        }
        
        swizzleScience = [NSFileManager jr_swizzleMethod:@selector(copyItemAtURL:toURL:error:) withMethod:@selector(ourCopyItemAtURL:toURL:error:) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"NSFileManager copyItemAtURL:toURL:error: replaced!");
        } else {
            
            NSLog(@"NSFileManager copyItemAtURL:toURL:error: failed to replace with error(: %@", theError);
        }
        
		#if TARGET_OS_TV

        NSString *suf = @"/System/Library/PrivateFrameworks/FrontBoard.framework";
        if ([[NSFileManager defaultManager] fileExistsAtPath:suf]){
            NSBundle *fb = [NSBundle bundleWithPath:suf];
            [fb load];
        }

        swizzleScience = [NSClassFromString(@"FBWorkspaceConnectionListener") jr_swizzleClassMethod:@selector(_sharedDomain) withClassMethod:@selector(_ourSharedDomain) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"FBWorkspaceConnectionListener _sharedDomain replaced!");
        } else {
            
            NSLog(@"FBWorkspaceConnectionListener _sharedDomain failed to replace with error(: %@", theError);
        }
        #else
        /*
         swizzleScience = [NSClassFromString(@"SpringBoardUI") jr_swizzleClassMethod:@selector(load) withClassMethod:@selector(ourLoad) error:&theError];
        if (swizzleScience == TRUE)
        {
            NSLog(@"SpringBoardUI +load replaced!");
        } else {
            
            NSLog(@"SpringBoardUI +load failed to replace with error(: %@", theError);
        }
        */
		#endif

		//DO NOT DO STUFF LIKE THIS, MY BOX DIED - I KNEW SWIZZLING CLUSTERS WAS BAD BUT HOLY SHIT.
        /*
         swizzleScience = [NSClassFromString(@"__NSPlaceholderArray") jr_swizzleMethod:@selector(initWithObjects:count:) withMethod:@selector(ourInitWithObjects:count:) error:&theError];
         if (swizzleScience == TRUE)
         {
         NSLog(@"NSArray initWithObjects:count: replaced!");
         } else {
         
         NSLog(@"NSArray initWithObjects:count: failed to replace with error(: %@", theError);
         }
         
         
         swizzleScience = [NSClassFromString(@"__NSArrayM") jr_swizzleMethod:@selector(removeObjectsInRange:) withMethod:@selector(ourRemoveObjectsInRange:) error:&theError];
         if (swizzleScience == TRUE)
         {
         NSLog(@"__NSArrayM removeObjectsInRange: replaced!");
         } else {
         
         NSLog(@"__NSArrayM removeObjectsInRange: failed to replace with error(: %@", theError);
         }
         */
        char * image=nil;
        BOOL writeToDisk=NO;
        ...
        if (recursive){
            ...
            if (error){
                NSLog(@"Could not create directory %@. Check permissions, error: %@",outputDir, error);
                exit(EXIT_FAILURE);
            }
            ...
            if (image){
                ...
                if (outputDir){
                    ...
                    imageString=[NSString stringWithUTF8String:image ];
